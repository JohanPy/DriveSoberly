...
import com.vaudibert.drivesoberly.domain.digestion.PhysicalBody
import com.vaudibert.drivesoberly.domain.digestion.Sex
...
internal class DigestionServiceTest {
    ...
    fun `BAC is zero during absorption lag`() {
        val now = Date()
        ingestBeer(now)
        // 1 second after ingestion: still within EMPTY lag window, so BAC is 0.
